...
func (s *Server) HandleBatchCreateTasks(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.Unmarshal(titiles, &input); err != nil {
		WriteError(w, "Invalid JSON", http.StatusBadRequest)
		return
	}
	...
	for i, title := range input.Titles {
		go func(t string, i int) {
			task, err := s.Store.Create(r.Context(), t)
			var errorMsg string
			if err != nil {
				errorMsg = err.Error()
			}
			ch <- batchResult{Task: task, Error: errorMsg, Index: i}
		}(title, i)
	}
